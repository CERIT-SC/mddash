...
- **Stop (`POST /jobs/{gmx,amber}/<id>/stop`)**: graceful, data-preserving counterpart of DELETE — deletes the K8s job with an extended grace period (`STOP_GRACE_PERIOD_SECONDS`), keeps the DB row, and sets `last_status=STOPPED` (re-reading the outcome first, so a run that beat the stop stays FINISHED). Terminal jobs (FINISHED/ERROR/STOPPED) are never flipped. **STOPPED is sticky**: `status` short-circuits on it without consulting K8s, because the Background-propagation deletion window keeps reporting the pod as RUNNING for up to the grace period — trusting that would resurrect the row, and the row intentionally outlives its K8s job (the poller only sweeps PENDING/RUNNING/UNKNOWN).
- **Pod-termination guarantees**: Kubernetes signals only PID 1. The sim command backgrounds the workload; `_sim_guard_block(sim_process)` forwards TERM via `pkill -x <sim_process>` — never to the MPI launcher, which treats TERM as "abort job" and kills ranks before `gmx mdrun` writes its signal checkpoint (`kill $MD_PID` is only the fallback when no sim process matched). The s3-sync sidecar traps TERM, waits (bounded, ~30s) for the `/data/job_completed` marker, then performs its final checksummed copy, so `delete_job(..., grace_period_seconds=60)` gives the checkpoint upload time to finish. A plain DELETE (grace 5s) does NOT guarantee the freshest checkpoint reaches S3.
