...
- **Gunicorn**: Production WSGI server with filtered access logging for successful health probes
...
- **Health access logs**: Successful `/api/health` probe access logs are suppressed via the Gunicorn logger class to avoid log congestion. Failed probes, 4xx/5xx responses, startup logs, and application errors must remain visible.
