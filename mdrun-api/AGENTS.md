...
- **Stop (`POST /jobs/{gmx,amber}/<id>/stop`)**: graceful, data-preserving counterpart of DELETE — deletes the K8s job with an extended grace period (`STOP_GRACE_PERIOD_SECONDS`), keeps the DB row, and sets `last_status=STOPPED`. Terminal jobs (FINISHED/ERROR/STOPPED) are never flipped. After the K8s job is gone, `status` falls back to `last_status`, so a stopped job reports `stopped` forever at zero polling cost (the poller only sweeps PENDING/RUNNING/UNKNOWN).
