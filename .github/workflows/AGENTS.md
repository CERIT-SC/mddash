...
- **`file://` dependency for mdrun-api**: `Chart.yaml` uses `file://../mdrun-api` so dev deploys resolve the sibling chart from local source instead of OCI. The `push-mddash-chart` Makefile target overrides this to `oci://` for prod packaging.
