...
- **Freeze covers publish, not just reads:** `publish()` 409s for any non-NULL `archive_state`, including the archiving window, because a publish submitted mid-archive would race an MDRepo draft + upload Job against the worker's `rm -rf`. (Notebook start keeps the narrower existing `archived_at` check. Archive submission already auto-stops running notebooks, and the wizard/card UI blocks starts during archiving.)

### Restore flow

1. **Gate checks:** experiment is archived, no job in flight, S3 configured, and the local experiment dir either **does not exist** or holds only a non-completed restore status doc, the retry sentinel that keeps failed/evicted restores re-submittable and lets the worker resume into partial data. Any other contents → 409 (clobber protection). The worker enforces the same gate.
...
- One shared filter list, `dashboard/rclone-filters.txt` (build context `dashboard/`), is baked into all three images (bisync sidecar, mdrepo uploader, archive worker) so "sync-worthy", "upload-worthy", and "archive-worthy" mean the same thing. Exclusions are global: `- /_archives/**` is mandatory for bisync (without it bisync downloads every archive back onto the PVC and resurrects purged prefixes) and a no-op elsewhere; status-doc exclusions likewise.
...
- The filter change triggers the existing filters-change recovery resync once per user pod at rollout (`setup_rclone` in `sync.sh`), the same sanctioned path used by previous filter edits; no new sync-loop code.
- Archive-worthiness is defined exactly once: the worker runs every copy/sync/check with `--filter-from /rclone-filters.txt`, so an archive contains precisely what bisync would sync (excludes `.binder-env` conda trees, checkpoints, `mwf_*` intermediates, status docs).
...
- **Restore into existing dir:** 409 unless the dir holds the restore retry sentinel (see Restore flow). A failed Job submission removes only archive's own queued doc (so a never-started archive can't freeze at `archiving`); restore's sentinel survives. Resubmission while the old Job still terminates returns 409 retry-later, not a 500.
