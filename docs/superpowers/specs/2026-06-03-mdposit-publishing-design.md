...
- Add simple target-aware publication state.
...
MDPosit/MDDB uses a guided manual handoff. MDDash generates a VRE Lite-compatible metadata file and provides the user with direct download links for the metadata file and the structure/topology/trajectory files required by the VRE Lite upload steps. The user opens `https://mdrepo.eu/vre_lite/`, uploads the metadata file first, reviews or fills the VRE Lite form, and uploads the prepared MD files through VRE Lite.
...
- `PublishStatus.EXPORTED`: MDDash prepared MDPosit files for handoff.
...
- Demo seed data should include at least one MDPosit-linked experiment.
- Demo publish flow should allow exercising MDPosit file handoff and accession linking without real MDPosit calls.
...
- Explicit `target=mdposit` routes to MDPosit file handoff.
...
4. Verify MDPosit file downloads, VRE Lite link, accession linking, and setup import paths.
