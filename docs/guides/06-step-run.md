...
- Stopping a GROMACS run resumes from the checkpoint the engine writes on the stop signal — no simulation progress is lost. Stopping AMBER keeps the trajectory up to the last sync, but a resume point exists only if the control file writes restarts periodically (`ntwr`).
...
- Only one *live* production job per simulation (extensions queue behind a stop/finish) — for parallel replicas create additional simulations via the **"New simulation"** button on the tab bar.
