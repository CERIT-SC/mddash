...
- Manifest `name` must be unique per experiment (it's the wizard tab identity); underscore-prefixed names are reserved for UI sentinels (`_new`). `create_app()` runs `Simulation.reconcile_duplicate_names()` at startup to rename pre-uniqueness duplicates/reserved names in place — jobs key on `simulation_path`, so only the JSON `name` field is rewritten.
