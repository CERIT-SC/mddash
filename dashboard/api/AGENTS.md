...
- **Extend** (`POST .../gmx/<path>/extend`, GMX only) submits a new segment with `-cpi <deffnm>.cpt -nsteps <cumulative>` on top of the manifest's extra_args (`utils.strip_run_control_args` strips `-nsteps`, rejects `-cpi` — the flow owns both flags). The cumulative anchor is **actual progress** (the previous segment's `nsteps_done`, then frozen for display), falling back to the target — a stopped segment resumes from where it stood. It reuses the previous segment's hardware config, requires a real checkpoint on PVC, and never deletes files: `mdrun -cpi` appends, keeping one continuous trajectory per simulation. The new row persists the cumulative `_nsteps`, which **outranks** the manifest `-nsteps` override in the `nsteps` property.
- **Appended logs**: `mdrun -cpi` appends each segment's block to the same `.log`. Full-file parsers (`_parse_nsteps`, `_parse_init_step`, `_parse_start_timestamp`) take the **last** match; tail parsers (`_parse_nsteps_done`, `_parse_performance`, `_parse_finish_timestamp`) use a **byte budget** (`utils.tail_bytes`) because the M-E-G-A-F-L-O-P-S/wallcycle trailer pushes markers past any line window. `performance` and the `nsteps_done` shortcut only trust a parse when `status == FINISHED` — a real TERM-stopped run also prints a `Performance:` line, and a live extension segment must not inherit the previous segment's trailer. Extend freezes the outgoing segment's `init_step`/`start_timestamp`/`performance`/`nsteps`/`nsteps_done` before submission so its history row keeps its own numbers.
