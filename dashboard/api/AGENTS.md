...
- **`step` IS the wizard phase index** (Setup 0, Tune 1, Run 2, Analyze 3) and `live` flags non-terminal tuner/production jobs (`JobStatus.is_live`); clients consume both directly — never decode ints or status strings client-side. A running job already advances to step 3 so partial trajectories can be analyzed mid-run; a STOPPED job also counts as step 3 "analyzing" (its data is kept). `Experiment._step_status` extends the same scale with publish = 4.

### Run lifecycle (stop / extend / segments)
- **Segment history**: a simulation accumulates one `simulation_jobs` row per run/extension — every segment-scoped route (`GET`, `stop`, `extend`, `log`, `submit`) acts on the **latest row by `created_at`**; `DELETE` is the only operation that cascades, wiping all segments' MDRun jobs, rows, and result files (that is the destructive "Re-run" path). At most one segment is live at a time.
- **Stop** (`POST .../gmx|amber/<path>/stop`) calls MDRun's stop endpoint and marks `_last_known_status=STOPPED` — terminal via `JobStatus.is_terminal`, never file-cleaning, rows kept.
- **Extend** (`POST .../gmx/<path>/extend`, GMX only) submits a new segment with `-cpi <deffnm>.cpt -nsteps <previous_cumulative + n>` composed onto the manifest's extra_args (`utils.strip_run_control_args` strips `-nsteps`, rejects `-cpi` — the flow owns both flags). It reuses the previous segment's hardware config, requires a real checkpoint on PVC, and never deletes files: `mdrun -cpi` appends, keeping one continuous trajectory per simulation. The new row persists the cumulative `_nsteps`, which **outranks** the manifest `-nsteps` override in the `nsteps` property.
- **Appended logs**: `mdrun -cpi` appends each segment's input-params/`Started mdrun` block to the same `.log`, so full-file parsers (`_parse_nsteps`, `_parse_init_step`, `_parse_start_timestamp`) take the **last** match; previous rows are protected by values cached before the log grew (extend freezes the latest segment's parses up front).
