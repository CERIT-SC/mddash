...
- **Paths inside a manifest are relative to the manifest file's own directory** (notebooks write manifests next to their outputs); `_resolve_files()`/`resolve_role()` rebase them to experiment-relative paths, with an existence-checked fallback to the experiment-relative interpretation. Consumers never see the manifest-relative form.
