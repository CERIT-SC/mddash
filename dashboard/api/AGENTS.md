...
- Experiment archive/restore/purge run as durable K8s Jobs mirroring the upload pattern (`archive/`, worker `dashboard/archive-worker`). Archives mirror the experiment at the bisync-excluded S3 prefix `_archives/<id>/`: filtered server-side seed + filtered `rclone sync` delta (sync's deletions keep re-archive passing the symmetric `rclone check` that gates local deletion). `.archive-status.json*` is filter-excluded from all copies/checks and bisync (paired root + `**/` lines) — an archived doc would overwrite the live restore sentinel; a `target-exists` refusal writes **no** doc (one would read as our retry sentinel and unlock clobbering). Durable state is DB columns reconciled in the schema's `pre_dump` (stash consumed by `archive_state`); Job liveness = no terminal condition (Pending is live), an in-flight doc with a dead Job reconciles to the direction's failed state. Restore never gets an API-side doc; a non-completed restore doc in a leftover dir is the retry sentinel (a live Job outranks it, anything else 409s as clobber protection). Gates carry `urn:mddash:archive-conflict`; `publish()` 409s for any non-NULL `archive_state`. Liveness lookups use `archive_status_cache` (1s TTL, written through on submission).
