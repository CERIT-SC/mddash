...
- Experiment archive/restore/purge run as durable K8s Jobs mirroring the upload pattern (`archive/`, worker image `dashboard/archive-worker`). Archives mirror the experiment at the bisync-excluded S3 prefix `_archives/<id>/` (server-side copy + filtered delta, so no re-upload of already-synced data); `rclone check` gates local deletion. `archived_at` is set/cleared by reconciliation in the schema's `pre_dump` — the durable markers are the DB columns, since the status doc disappears with the dir and Jobs TTL-expire; a status doc claiming in-flight work with no live Job (eviction is unretried, `backoffLimit: 0`) reconciles to the direction's failed state, and failed Job submission removes its queued doc. The restore flow never writes an API-side status doc (it would recreate the dir the worker copies into); a FAILED restore doc in the leftover dir is the retry sentinel — `restore()` and the worker treat it as continuation, anything else in a present dir 409s/fails as clobber protection. Job-liveness lookups go through `archive_status_cache` (1s TTL) because `pre_dump` runs per serialized experiment.
