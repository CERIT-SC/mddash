...
- Demo state persists at `MDDASH_DEMO_DATA_DIR` (default `/tmp/mddash`): on restart the seed path is skipped and `_rehydrate_runtime_state()` rebuilds in-memory job state from the SQLite DB. Rehydration overwrites read-only-locked manifests and keeps running jobs running. Wipe the dir for a pristine seed.
- Rehydration runs on EVERY debug-reload restart (parent + child), so any file-state the seed depends on must also be applied in `_rehydrate_runtime_state()`. Example: the placeholder enzyme manifests are back-dated in `_backdate_stale_simulations()` so the job-linked `md` simulation stays the experiment's most recent activity and drives its step (identical mtimes from the tight write loop would otherwise pick the alphabetically-first manifest).
- Rehydrated RUNNING jobs get a fresh `created_at` + 600s duration so they don't insta-finish, plus a fresh DB `start_timestamp` so `estimated_time` isn't inflated from the previous session.
...
- Which seeded analyses stay RUNNING is defined once in `RUNNING_SEEDED_ANALYSIS_IDS` (shared by the seed and rehydration); their `demo_state` entries carry `created_at` like the `_create_job` mock — without it the status flip-flops ERROR→FINISHED on poll because no result files are written for them.
