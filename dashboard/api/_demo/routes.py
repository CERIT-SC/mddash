...
def create_experiment() -> Response:
    ...
    if exp_type == "pdb":
        ...
        uploaded_file = request.files.get("simulation-file")
        file_name = uploaded_file.filename if uploaded_file else "unknown.tpr"
        source_message = f"Created by uploading TPR file '{file_name}'."
        has_tpr = bool(file_name and file_name.endswith(".tpr"))
...
def mdrepo_status() -> Response:
    """Check MDRepo authentication status."""
    return ApiResponse.success(
        {"authenticated": state.mdrepo_authenticated, "mdrepo_url": "https://workflow-repo.test.du.cesnet.cz"}
    )


@bp.route("/api/mdrepo/auth", methods=["GET"])
def mdrepo_auth() -> WerkzeugResponse:
...
demo_files = [
    ...
    {
        "name": "ASuperLongTPRNameToTestTheUICapabilities.tpr",
        "url": "http://localhost:8888/api/experiments/aaaaa/files/ASuperLongTPRNameToTestTheUICapabilities.tpr",
        "size": 514912,
    },
    {"name": "md.tpr", "url": "http://localhost:8888/api/experiments/aaaaa/files/md.tpr", "size": 321654},
    {
        "name": "trajectory.xtc",
        "url": "http://localhost:8888/api/experiments/aaaaa/files/trajectory.xtc",
        "size": 3038996,
    },
...
def list_experiment_files(experiment_id: str) -> Response:
