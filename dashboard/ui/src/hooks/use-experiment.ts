...
import { get_experiment, get_experiment_step, edit_experiment } from "@/util/api";
...
export function useExperimentStep(id: string, currentStep: number) {
    ...
    useEffect(() => {
        if (query.data !== undefined && query.data !== prevStepRef.current) {
            ...
            queryClient.setQueryData<Experiment>(["experiment", id], (old) =>
                old ? { ...old, step: query.data! } : old,
            );
        }
