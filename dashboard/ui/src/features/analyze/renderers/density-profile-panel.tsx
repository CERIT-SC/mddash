...
import type { DensityProfileAnalysis } from "../analysis-types"
...
const DensityProfilePanel: FC<{ data: DensityProfileAnalysis }> = ({ data }) => {
  ...
  const chartSeries = useMemo(() => {
    ...
    components.forEach((component) => {
      ...
      const subSeries = buildLineSeries(component.name, zPositions, values)
      if (subSeries) {
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="border-border rounded-md border p-3">
          <p className="text-text-muted/80 text-[11px] tracking-wide uppercase">Samples</p>
          <p className="text-lg font-semibold">{zPositions.length}</p>
        </div>
        <div className="border-border rounded-md border p-3">
          <p className="text-text-muted/80 text-[11px] tracking-wide uppercase">Components</p>
          <p className="text-lg font-semibold">{components.length}</p>
        </div>
        <div className="border-border rounded-md border p-3">
          <p className="text-text-muted/80 text-[11px] tracking-wide uppercase">Metric</p>
          <p className="text-lg font-semibold">{metricLabel}</p>
        </div>
        <div className="border-border rounded-md border p-3">
          <p className="text-text-muted/80 text-[11px] tracking-wide uppercase">Mode</p>
      ...
      <div className="border-border flex min-h-80 flex-col rounded-lg border p-4">
        <div className="mb-3 flex items-center justify-between">
          ...
          {selectedMetric?.unit && <p className="text-text-muted text-xs">Units: {selectedMetric.unit}</p>}
        </div>
      ...
      <div className="border-border rounded-lg border p-4">
        <p className="text-text-muted mb-3 text-xs tracking-wide uppercase">Components overview</p>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-text-muted text-xs">Selection size: {component.selectionCount || "N/A"}</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-text-muted text-xs">
                    Peak {modeLabel.toLowerCase()}:
                    <span className="text-text ml-1 font-semibold">{`${peakValue}${suffix}`}</span>
                  </div>
