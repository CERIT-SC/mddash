...
import { downsampleMatrix, HeatmapMatrix } from "../charts"
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-muted text-xs">Highlights residues participating in each agent interface.</p>
        </div>
        {entries.length > 1 && (
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="border-border space-y-2 rounded-lg border p-4">
          ...
          <div>
            <p className="text-text-muted mb-1 text-xs tracking-wide uppercase">Interface residues snapshot</p>
            <div className="text-text-muted grid grid-cols-1 gap-2 text-xs">
              <div>
                <p className="text-text mb-1 font-medium">{selected.agent1}</p>
                <p>{chunkList(interfaceList1.values, interfaceList1.label).join(", ") || "None"}</p>
              </div>
              <div>
                <p className="text-text mb-1 font-medium">{selected.agent2}</p>
                <p>{chunkList(interfaceList2.values, interfaceList2.label).join(", ") || "None"}</p>
              </div>
            </div>
          </div>
        </div>
        <div className="border-border flex min-h-[280px] flex-col rounded-lg border p-4 lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            ...
            <p className="text-text-muted text-xs">Cells: {heatmap.data.length}</p>
          </div>
