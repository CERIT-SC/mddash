...
import type { TMScoresAnalysis } from "../analysis-types"
import { downsampleSeries, formatStatValue, LineChart } from "../charts"
...
const TMScoresChart: FC<TMScoresChartProps> = ({ data }) => {
  ...
  const references = useMemo(() => Array.from(new Set(data.data.map((entry) => entry.reference))), [data.data])
  ...
  if (series.length === 0) {
    return (
      <div className="text-text-muted flex h-full items-center justify-center text-sm">No TM-score data available.</div>
    )
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      <div className="flex flex-wrap items-center gap-6">
        ...
        {statsList.length > 0 && (
          <div className="text-text-muted flex flex-wrap items-center gap-x-6 gap-y-1 text-xs">
      ...
      <div className="min-h-0 flex-1">
        <LineChart series={series} xLabel="Frame" yLabel="TM-score" yScale showLegend />
      </div>
    </div>
  )
}
