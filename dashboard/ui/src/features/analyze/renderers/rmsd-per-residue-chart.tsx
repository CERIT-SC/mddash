...
import type { RMSDPerResidueAnalysis } from "../analysis-types"
import { buildTimeSeries, formatStatValue, LineChart } from "../charts"
...
const RMSDPerResidueChart: FC<RMSDPerResidueChartProps> = ({ data }) => {
  ...
  const activeEntry = useMemo(() => {
    ...
    return entries.find((entry) => entry.name === selectedDataset) ?? entries[0]
  }, [entries, selectedDataset])
        ...
        {stats && (
          <div className="text-text-muted flex flex-wrap items-center gap-x-6 gap-y-1 text-xs">
            <div className="flex items-center gap-3">
              <span className="text-text/80 font-medium">{activeEntry?.name}</span>
