...
import type { CoordsFormat, StructureFormat } from "./mol-star-formats"
...
export default function MolStar(props: MolStarProps) {
  const { width = "500px", height = "500px", structureUrl, structureFormat, coordsUrl, coordsFormat } = props
    ...
    const init = async () => {
      ...
      try {
        ...
        if (structureUrl && coordsUrl) {
          ...
          throw new Error("No structure source provided (structureUrl, or structureUrl+coordsUrl)")
        }
      } catch (error) {
        if (isMountedRef.current && !cancelled) {
  ...
  return (
    ...
    <div style={{ width, height, position: "relative", zIndex: 60 }}>
      {loading && (
        ...
        >
          <Loader2 className="text-text-muted h-8 w-8 animate-spin" aria-label="Loading structure" />
        </div>
      )}
      {error !== null && !loading && (
        <div className="border-border bg-surface absolute inset-0 z-20 flex flex-col items-center justify-center gap-1 rounded-lg border p-6 text-center">
          <p className="text-error text-sm font-medium">The structure could not be loaded</p>
