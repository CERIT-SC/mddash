...
import type { AreaPerLipidAnalysis } from "@/util/analysis-types"
...
const AreaPerLipidPanel: FC<{ data: AreaPerLipidAnalysis }> = ({ data }) => {
  ...
  const medianArray = useMemo(() => toNumericArray(areaData?.median), [areaData?.median])
  const stdArray = useMemo(() => toNumericArray(areaData?.std), [areaData?.std])
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Median area</p>
          <p className="text-lg font-semibold">{formatStat(medianSummary)} Å²</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Std. deviation</p>
          <p className="text-lg font-semibold">{formatStat(stdSummary)} Å²</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Grid resolution</p>
          <p className="text-lg font-semibold">{gridResolution}</p>
        </div>
      </div>

      {medianSeries.length ? (
        <div className="border-border/60 h-64 rounded-lg border p-4">
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-2">
        <div className="border-border/60 flex min-h-80 flex-col gap-3 rounded-lg border p-4">
          <div>
            <p className="text-sm font-medium">Upper leaflet grid</p>
            <p className="text-muted-foreground text-xs">Per-cell area contributions for the upper leaflet</p>
          </div>
        ...
        <div className="border-border/60 flex min-h-80 flex-col gap-3 rounded-lg border p-4">
          <div>
            ...
            <p className="text-muted-foreground text-xs">Mirrored area map for the lower leaflet</p>
          </div>
          <div className="min-h-70 flex-1">
            {lowerHeatmap.length ? (
              <HeatmapMatrix
