    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Clusters</p>
          <p className="text-base font-semibold">{clusters.length}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Total frames</p>
          <p className="text-base font-semibold">{totalFrames}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Cutoff</p>
          <p className="text-base font-semibold">{typeof data.cutoff === "number" ? data.cutoff : "N/A"}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Version</p>
            ...
            {transitionHeatmap?.triples.length ? (
              <HeatmapMatrix
                ...
                tooltipFormatter={(context) => {
                  const fromLabel = labels[context.yIndex] ?? `Cluster ${context.yIndex + 1}`
                  const toLabel = labels[context.xIndex] ?? `Cluster ${context.xIndex + 1}`
      ...
      <div className="border-border/70 rounded-lg border p-4">
        ...
        <div className="mt-3 space-y-2">
          {topTransitions.length ? (
            ...
            <div className="border-muted-foreground/40 text-muted-foreground rounded-md border border-dashed px-3 py-4 text-sm">
