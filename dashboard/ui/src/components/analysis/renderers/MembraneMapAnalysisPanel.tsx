...
import type { MembraneMapAnalysis } from "@/util/analysis-types"
...
const buildTooltipFormatter =
  ...
  (payload: unknown): string => {
    ...
    const header = normalized[0].axisValueLabel ?? categories[normalized[0].dataIndex ?? 0] ?? "Membrane"
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-5">
        <div className="border-border/60 flex min-h-80 flex-col gap-4 rounded-lg border p-4 lg:col-span-3">
          <div className="grid gap-2 sm:grid-cols-3">
            <div className="border-border/70 rounded-md border p-3">
              <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Membranes shown</p>
              <p className="text-lg font-semibold">{filteredMembranes.length || 0}</p>
            </div>
            <div className="border-border/70 rounded-md border p-3">
              <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Top lipids</p>
              <p className="text-lg font-semibold">{stats.top}</p>
            </div>
            <div className="border-border/70 rounded-md border p-3">
              <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Bottom lipids</p>
        ...
        <div className="flex flex-col gap-4 lg:col-span-2">
          <div className="border-border/60 space-y-4 rounded-lg border p-4">
            ...
            {filteredMembranes.map((membrane) => (
              <div key={membrane.id} className="space-y-2">
                {hasMultipleMembranes && (
                  <p className="text-muted-foreground text-[11px] tracking-wide uppercase">{membrane.label}</p>
                )}
                <LabeledList
                  label="Top leaflet"
          ...
          <div className="border-border/60 space-y-3 rounded-lg border p-4">
            ...
            {unassignedLipids.length ? (
              ...
              <p className="text-muted-foreground text-xs">No unassigned lipids reported.</p>
            )}
          </div>
        </div>
      </div>
