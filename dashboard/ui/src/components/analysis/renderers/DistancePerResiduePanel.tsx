...
import type { DistancePerResidueAnalysis } from "@/util/analysis-types"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { downsampleMatrix, formatStatValue, HeatmapMatrix } from "@/components/charts"
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-4 xl:grid-cols-4">
        <div className="border-border/60 space-y-3 rounded-lg border p-4">
          ...
          {matrixStats && (
            <div className="text-muted-foreground space-y-1 text-xs">
              <p>
                Avg: <span className="text-foreground font-medium">{formatStatValue(matrixStats.average)}</span>
              </p>
              <p>
                Min: <span className="text-foreground font-medium">{formatStatValue(matrixStats.min)}</span>
              </p>
              <p>
                Max: <span className="text-foreground font-medium">{formatStatValue(matrixStats.max)}</span>
              </p>
            </div>
          )}
          {topPairs.length > 0 && (
            <div>
              <p className="text-muted-foreground mb-1 text-xs tracking-wide uppercase">Closest residue pairs</p>
              <ul className="text-muted-foreground space-y-1 text-xs">
                {topPairs.map((pair, index) => (
                  <li key={`${pair.row}-${pair.col}`}>
                    #{index + 1} · A1 {pair.col + 1} ↔ A2 {pair.row + 1}:{" "}
                    <span className="text-foreground font-medium">{formatStatValue(pair.value)} A</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-border/60 flex min-h-80 flex-col rounded-lg border p-4 xl:col-span-3">
