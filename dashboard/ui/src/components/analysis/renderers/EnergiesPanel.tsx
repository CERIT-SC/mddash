...
import type { EnergiesAgentData, EnergiesAnalysis } from "@/util/analysis-types"
...
const buildLineSeries = (label: string, xValues: number[], yValues: number[]): LineSeries | undefined => {
...
const buildStackedSeries = (label: string, xValues: number[], yValues: number[]): StackedSeries | undefined => {
  if (!xValues.length || !yValues.length) return undefined
  const limit = Math.min(xValues.length, yValues.length)
  const data: Array<[number, number]> = []
  for (let i = 0; i < limit; i += 1) {
    ...
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Residues</p>
          <p className="text-lg font-semibold">{labels.length}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Stage</p>
          <p className="text-lg font-semibold">{stageLabel}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Agent</p>
          <p className="text-lg font-semibold">{agentKey === "agent1" ? "Agent 1" : "Agent 2"}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Top |S|</p>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="border-border/60 flex min-h-70 flex-col rounded-lg border p-4">
        ...
        <div className="border-border/60 rounded-lg border p-4">
          ...
          {topContributors.length ? (
            ...
            <p className="text-muted-foreground text-xs">Unable to compute contributor ranking for this selection.</p>
          )}
        </div>
      </div>
    </div>
