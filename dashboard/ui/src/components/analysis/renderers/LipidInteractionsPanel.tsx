...
import type { LipidInteractionAnalysis } from "@/util/analysis-types"
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Residues tracked</p>
          <p className="text-lg font-semibold">{residueIndices.length}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Lipid species</p>
          <p className="text-lg font-semibold">{lipidEntries.length}</p>
        </div>
        <div className="border-border/70 rounded-md border p-3">
          <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">Peak occupancy</p>
          <p className="text-lg font-semibold">{formatNumber(maxValue)}</p>
        </div>
      </div>

      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-5">
        <div className="border-border/60 flex min-h-90 flex-col gap-4 rounded-lg border p-4 lg:col-span-3">
        ...
        <div className="flex flex-col gap-4 lg:col-span-2">
          <div className="border-border/60 space-y-3 rounded-lg border p-4">
            <div>
              ...
              <p className="text-muted-foreground text-xs">Ranked by cumulative lipid contact counts</p>
            </div>
            {topResidues.length ? (
              <LabeledList label="Residues" list={topResidues} orientation="vertical" maxVisibleItems={6} />
            ) : (
              <p className="text-muted-foreground text-xs">No residue interactions to highlight.</p>
            )}
          </div>
          <div className="border-border/60 space-y-3 rounded-lg border p-4">
            <div>
              <p className="text-sm font-medium">Top lipid species</p>
              <p className="text-muted-foreground text-xs">Summed occupancies across all residues</p>
            </div>
            {topLipids.length ? (
              <LabeledList label="Lipids" list={topLipids} orientation="vertical" maxVisibleItems={6} />
            ) : (
              <p className="text-muted-foreground text-xs">No lipid species exceed the selected filter.</p>
            )}
          </div>
        </div>
      </div>
