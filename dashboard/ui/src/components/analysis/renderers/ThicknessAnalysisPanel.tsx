...
import type { ThicknessAnalysis } from "@/util/analysis-types"
...
const buildSeries = (label: string, frames: number[], values: number[]): LineSeries | undefined => {
...
const ThicknessAnalysisPanel: FC<{ data: ThicknessAnalysis }> = ({ data }) => {
  ...
  const thicknessSeries = useMemo(() => buildSeries("Thickness", frames, thicknessValues), [frames, thicknessValues])
  const positiveSeries = useMemo(() => buildSeries("Mean positive", frames, meanPositive), [frames, meanPositive])
  const negativeSeries = useMemo(() => buildSeries("Mean negative", frames, meanNegative), [frames, meanNegative])
  ...
  const midplaneSeries = useMemo(() => buildSeries("Midplane Z", frames, midplaneValues), [frames, midplaneValues])
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {infoCards.map((card) => (
          <div key={card.label} className="border-border/70 rounded-md border p-3">
            <p className="text-muted-foreground/80 text-[11px] tracking-wide uppercase">{card.label}</p>
            <p className="text-lg font-semibold">{card.value}</p>
          </div>
        ))}
      </div>

      <div className="border-border/60 flex min-h-[340px] flex-col gap-3 rounded-lg border p-4">
        <div>
          <p className="text-sm font-medium">Leaflet separation over time</p>
          <p className="text-muted-foreground text-xs">Overall thickness with positive/negative leaflet centroids</p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="border-border/60 flex min-h-[240px] flex-col gap-3 rounded-lg border p-4">
          <div>
            <p className="text-sm font-medium">Midplane drift</p>
            <p className="text-muted-foreground text-xs">Z-position of the bilayer midplane per frame</p>
          </div>
        ...
        <div className="border-border/60 space-y-3 rounded-lg border p-4">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">σ (positive leaflet)</span>
              <span className="font-medium">{`${formatStat(average(stdPositive))} Å`}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">σ (negative leaflet)</span>
