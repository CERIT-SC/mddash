...
import type { HydrogenBondsAnalysis } from "@/util/analysis-types"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
import { buildTimeSeries, downsampleMatrix, formatStatValue, HeatmapMatrix, LineChart } from "@/components/charts"
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        <div className="border-border/60 space-y-3 rounded-lg border p-4">
          <div>
            <p className="text-muted-foreground text-xs tracking-wide uppercase">Interaction</p>
          ...
          {topBonds.length > 0 && (
            <div>
              <p className="text-muted-foreground mb-1 text-xs tracking-wide uppercase">Top occupancy bonds</p>
              <ul className="text-muted-foreground space-y-1 text-xs">
                {topBonds.map((bond) => (
                  <li key={bond.index}>
                    ...
                    :<span className="text-foreground font-medium">{`${(bond.occupancy * 100).toFixed(1)}%`}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="border-border/60 flex min-h-[280px] flex-col rounded-lg border p-4 xl:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium">Bonds formed per frame</p>
              <p className="text-muted-foreground text-xs">Count of hydrogen bonds active at each frame.</p>
            </div>
            <p className="text-muted-foreground text-xs">Frames: {countsPerFrame.length || 0}</p>
          </div>
      ...
      <div className="border-border/60 flex min-h-[320px] flex-col rounded-lg border p-4">
        <div className="mb-3 flex items-center justify-between">
          <div>
          ...
          <p className="text-muted-foreground text-xs">Cells: {heatmapPayload.data.length}</p>
        </div>
