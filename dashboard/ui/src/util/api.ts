...
const parseResponse = <T>(response: AxiosResponse, fallbackMsg: string): ApiData<T> => {
  ...
  const errMsg = failed ? response.data.message || fallbackMsg : null
  const data = response.data.data
  return { data: data, error: errMsg }
}

const handle_request = async <T>(request: Promise<AxiosResponse>, fallbackMsg: string): Promise<ApiData<T>> => {
  try {
...
export const get_experiments = async (): Promise<ApiData<Experiment[]>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments`), "Failed to fetch experiments.")
}

export const get_experiment = async (id: string): Promise<ApiData<Experiment>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments/${id}`), "Failed to fetch experiment.")
}

export const create_experiment = async (formData: FormData): Promise<ApiData<Experiment>> => {
  return await handle_request(axios.post(`${API_BASE}/experiments`, formData), "Failed to create experiment.")
}

export const delete_experiment = async (id: string): Promise<ApiData<null>> => {
  return await handle_request(axios.delete(`${API_BASE}/experiments/${id}`), "Failed to delete experiment.")
}

export const edit_experiment = async (id: string, data: object): Promise<ApiData<Experiment>> => {
  return await handle_request(axios.patch(`${API_BASE}/experiments/${id}`, data), "Failed to edit experiment.")
}
...
export const publish_experiment = async (id: string): Promise<ApiData<PublishResponse>> => {
  return await handle_request(axios.post(`${API_BASE}/experiments/${id}/publish`), "Failed to publish experiment.")
}

export const get_experiment_step = async (id: string): Promise<ApiData<number>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments/${id}/step`), "Failed to fetch experiment step.")
}
...
export const get_mdrepo_status = async (): Promise<ApiData<MDRepoStatus>> => {
  return await handle_request(axios.get(`${API_BASE}/mdrepo/status`), "Failed to check MDRepo status.")
}
...
export const get_notebook = async (id: string): Promise<ApiData<Notebook>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments/${id}/notebook`), "Failed to fetch notebook.")
}

export const spawn_notebook = async (id: string): Promise<ApiData<Notebook>> => {
  return await handle_request(axios.post(`${API_BASE}/experiments/${id}/notebook`), "Failed to spawn notebook.")
}

export const delete_notebook = async (id: string): Promise<ApiData<null>> => {
  return await handle_request(axios.delete(`${API_BASE}/experiments/${id}/notebook`), "Failed to delete notebook.")
}

// ----- Tuner -----

export const tuner_statuses = async (id: string): Promise<ApiData<TunerJob[]>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments/${id}/tuner`), "Failed to fetch tuner statuses.")
}
...
export const delete_tuner = async (id: string, tprName: string): Promise<ApiData<null>> => {
  return await handle_request(axios.delete(`${API_BASE}/experiments/${id}/tuner/${tprName}`), "Failed to kill tuner.")
}
...
export const gmx_statuses = async (id: string): Promise<ApiData<GromacsJob[]>> => {
  return await handle_request(axios.get(`${API_BASE}/experiments/${id}/gmx`), "Failed to fetch Gromacs statuses.")
}
...
export const gmx_logs = async (
  id: string,
  tprName: string,
  type: "gmx" | "stdout" | "stderr",
  tail: number
...
export const get_metrics = async (): Promise<ApiData<ResourceUsage>> => {
  return await handle_request(axios.get(`${API_BASE}/metrics`), "Failed to fetch metrics.")
}
