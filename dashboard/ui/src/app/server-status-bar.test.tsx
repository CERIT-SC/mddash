...
describe("ServerStatusBar", () => {
  ...
  it("confirms before sending the browser to the hub home page to stop the server", async () => {
    stubMetricsFetch()
    const assign = vi.fn()
    ...
    // Destructive action must confirm first — no navigation yet.
    expect(assign).not.toHaveBeenCalled()
    const dialog = await screen.findByRole("alertdialog")
    await user.click(within(dialog).getByRole("button", { name: "Stop server" }))
