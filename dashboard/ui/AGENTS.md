...
- Backend returns resources directly — `r.data` is the payload (no envelope); the axios error interceptor builds an `ApiError` from RFC 9457 problem-details responses (`{type, title, detail}` with `Content-Type: application/problem+json`). `error.message` is set to `detail` so `toast.error(error.message)` works unchanged. A React `ErrorBoundary` wraps the app root for render-time crashes.
