...
- **Wizard state lives in the URL** (`/$id/wizard?tab=<simulation name>&step=<0-4>`, validated by the route's `validateSearch`); tab `_new` is the create-mode sentinel. Each `.simulation.json` owns its step — the API infers `step`/`status` per manifest (jobs filtered by `simulation_path`); the experiment only inherits its latest simulation's step (publishing stays experiment-level). `WizardStepper` is a pure URL-driven shell: tabs wrap the stepper, steps receive `{ experiment, simulation, goToStep }`, and the shell polls `useSimulations` (5 s) instead of the deleted `/experiments/{id}/step` endpoint + `useExperimentStep`. Publish unlocks once any simulation reaches step 4 (published/publishing override kept). `DEBUG` mode shows a "DEBUG: next step" button.
